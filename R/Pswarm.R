Pswarm = function(DataOrDistance, Cls = NULL, QuadOrHexa = "Hexa", NumJumps = 4,
                  ColumnsLines = NULL, Parallel = FALSE, NCores = "max",
  ...
  # ColumnsLines                Numeric vector with two entries: Number of lines
  ...
  # ColumnsLines[1:2]            Numeric vector with number of lines and columns
  #                              of the 2D projection plane
  ...
  # ColumnsLines is NULL or numerical vector of size 2 (if not, set ColumnsLines = c(NULL,NULL) # Global variable: Lines and Columns of grid)
  if(!is.null(ColumnsLines)){
    if(!is.vector(ColumnsLines) | (length(ColumnsLines) != 2)){
      stop("ColumnsLines is not a numeric vector of length 2.")
    }
  }
  ...
  if(is.null(ColumnsLines)){                # Global variable: Lines and Columns of grid; ColumnsLines=[X,Y]
    ColumnsLines = setGridSize(InputDistances = MyDistanceMatrix, minp = 0.01, maxp = 0.99, Verbose = Verbose)
  }
  ...
  Columns     = ColumnsLines[1]             # Columns of grid
  Lines       = ColumnsLines[2]             # Lines of grid
  ...
  ## Geht schief wenn ColumnsLines zu klein
  ...
  # Rmin: minimum radius depending on grid size (ColumnsLines) and p=0.05 (assuming uniform distribution of databots)
  ...
  if(length(ShiftReIdx) > 0){
    bmu[ShiftReIdx, 1] = Columns
  }
  if(length(ShiftImIdx) > 0){
    bmu[ShiftImIdx, 2] = Lines
  }

setPolarGrid = function(Lines, Columns, QuadOrHexa = "Hexa", PlotIt = FALSE,
                        global = TRUE){
  # V = setPolarGrid (Lines, Columns)
  # V = setPolarGrid (Lines, Columns, QuadOrHexa = F, PlotIt = F)
  # 
  # DESCRIPTION
  # ???
  # 
  ...
  # 
  # OPTINAL
  # quadOrHex     Boolean, If False Hexagonal grid, default quad grid
  # PlotIt        Boolean
  # global        Boolean, Wie sollen die moeglichen Radien bestimmt werden
  # 
  # OUTPUT list V with
  # V$GridRadii[Lines,Columns]                  Radii Matrix of all possible
  #                                             Positions of DataBots in Grid
  # V$GridAngle[Lines,Columns]                  Angle Matrix of all possible
  #                                             Positions of DataBots in Grid
  # V$AllallowedDBPosR0[Lines+1,Lines+1]        Matrix of radii in polar
  #                                             coordinates respecting origin
  #                                             (0,0) of all allowed DataBots
  #                                             Positions in one jump
  # V$AllallowedDBPosPhi0[Lines+1,Lines+1]      Matrix of angle in polar
  #                                             coordinates respecting origin
  #                                             (0,0) of all allowed DataBots
  #                                             Positions in one jump
  # 
  # 1st Author: MT 12/2014
  # 2nd Author: QS 2022
  # 
  # TODO
  ...
  if(isTRUE(QuadOrHexa)){
    QuadOrHexa = "Quad"
  }
  
  if(isFALSE(QuadOrHexa)){
    QuadOrHexa = "Hexa"
  }
  ...
  }else if (QuadOrHexa == "Hexa"){ #Or in Hexadiagonal
    #Generierung alles Moeglichen Position bezueglch Urpsrung in polaren Koordinaten
    #################
    if(global){
      if(Columns %%4 ==0){
...
#       PositionPhi0=matrix(posPhi2,nrow=Lines+1,ncol=Lines+1,byrow=TRUE)
    ...
    stop("Undefined input for QuadOrHexa")
  }
  ...
  if(PlotIt){   
   # tryCatch({requireNamespace('plotrix')},error=function(ex) {})#zum plotten     
    ...
    x      = c(gColumns/2, gColumns, gColumns, gColumns, gColumns/2, 0, 0, 0, gColumns/2)
  ...
  return(list("GridRadii"           = MatrixR,
              "GridAngle"           = MatrixW,
              "AllallowedDBPosR0"   = PositionR0,
              "AllallowedDBPosPhi0" = PositionPhi0,
              "Binary"              = binary))
